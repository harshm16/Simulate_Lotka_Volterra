    ...
    def Class_EMBasedOnDegree(self, image, degree):
        ...
        for img_x in range(image.shape[0]):
            for img_y in range(image.shape[1]):
                ...
                #{
                #for Lotka Volterra
                sol = solve_ivp(lotka,  [0,t[-1]], [x_prime, y_prime], method='RK45', t_eval = t, args=(alpha, beta, gamma, delta))
                
                #for rotation
                # sol = solve_ivp(rotate, [0,t[-1]], [x_prime, y_prime], method='RK45', t_eval = t, args=(alpha, beta, gamma, delta))
